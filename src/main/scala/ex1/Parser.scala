...
trait NotTwoConsecutive[T] extends Parser[T]:
  ...
  override abstract def parse(t: T): Boolean =
    val equalsPrevious = t == last
    last = t
    !equalsPrevious && super.parse(t)
...
@main def checkParsers(): Unit =
  ...
  println("\n" + parserNE.parseAll("0101".toList)) // true
  ...
  println("\n" + parserNTC.parseAll("XYZ".toList)) // true
